...
class KalshiMarketPrice(Base):
    """Time-series price snapshots for Kalshi markets."""

    __tablename__ = "kalshi_market_prices"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    market_ticker: Mapped[str] = mapped_column(String(100), nullable=False, index=True)
    ...
    __table_args__ = (
        Index("ix_kalshi_prices_ticker_timestamp", "market_ticker", "timestamp"),
    )

    def __repr__(self) -> str:
        return (
            f"<KalshiMarketPrice(id={self.id}, ticker='{self.market_ticker}', "
            f"yes={self.yes_price:.3f}, timestamp={self.timestamp})>"
        )


class CrossPlatformSignal(Base):
    """Aggregated cross-platform signals comparing Kalshi prices to external forecasts."""

    __tablename__ = "cross_platform_signals"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    event_description: Mapped[str] = mapped_column(String(500), nullable=False)
    kalshi_ticker: Mapped[str] = mapped_column(String(100), nullable=False, index=True)
    kalshi_price: Mapped[float] = mapped_column(nullable=False)
    polymarket_price: Mapped[float] = mapped_column(nullable=True)
    metaculus_forecast: Mapped[float] = mapped_column(nullable=True)
    # abs(kalshi_price - consensus) where consensus is weighted avg of available external signals
    divergence_score: Mapped[float] = mapped_column(nullable=False)
    timestamp: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False, index=True
    )
    created_at: Mapped[datetime] = mapped_column(
    ...
    __table_args__ = (
        Index("ix_cross_platform_signals_ticker_timestamp", "kalshi_ticker", "timestamp"),
        Index("ix_cross_platform_signals_divergence", "divergence_score"),
    )

    def __repr__(self) -> str:
        return (
            f"<CrossPlatformSignal(id={self.id}, ticker='{self.kalshi_ticker}', "
            f"divergence={self.divergence_score:.3f})>"
        )


class PaperTrade(Base):
    """Paper trades generated by cross-platform signal analysis."""
    ...
    size_usd: Mapped[float] = mapped_column(nullable=False, default=100.0)
    ...
    resolution_result: Mapped[str] = mapped_column(String(10), nullable=True)  # "WIN", "LOSS", "PUSH"
    pnl: Mapped[float] = mapped_column(nullable=True)
    strategy_tag: Mapped[str] = mapped_column(String(50), nullable=False, index=True)
    ...
    __table_args__ = (
        Index("ix_paper_trades_ticker_open", "market_ticker", "is_open"),
        Index("ix_paper_trades_strategy_result", "strategy_tag", "resolution_result"),
    )

    def __repr__(self) -> str:
        return (
            f"<PaperTrade(id={self.id}, ticker='{self.market_ticker}', "
            f"side={self.side}, entry={self.entry_price:.3f}, open={self.is_open})>"
        )
